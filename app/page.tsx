...
export default function Home() {
  ...
  useEffect(() => {
    // Redireciona imediatamente se não houver token após o loading
    if (!loading && !token) {
      router.replace('/login');
    }
  }, [loading, token, router]);

  // Mostra loading enquanto verifica autenticação OU se não houver token ainda
  if (loading || !token) {
    return (
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-blue-700">
              ...
              <span className="font-bold text-blue-900">{creditos}</span>
            </p>
          </div>
